  ...
  module Net
    class WebRequest
      ...
      def perform_async_get( parse_response=nil, &callback)
        self.begin_get_response AsyncCallback.new{ |arequest|
          begin
            res = self.end_get_response(arequest)
            using(rdr = StreamReader.new(res.get_response_stream)) do
      ...
      def handle_exception(e)
        if e.status == WebExceptionStatus.protocol_error
          ...
          when HttpStatusCode.NotModified: # 304 Not modified = no new tweets so ignore error.
            break
          when HttpStatusCode.BadRequest:
...
class RequestLimitException < System::Exception
  def initialize(message = "You've made too many requests. Twitter only allows 70 requests/hour.")
    super(message)
  end
end

...
module WpfBehavior
  ...
  module Databinding
    ...
    def self.included(base)
      unless base.ancestors.include? ComponentModel::INotifyPropertyChanged
        base.send :include, ComponentModel::INotifyPropertyChanged 
      end
    ...
    def raise_property_changed(name)
      ...
      @__handlers__.each do |ev|
        ev.invoke self, PropertyChangedEventArgs.new(name) if ev.respond_to? :invoke
        ev.call self, PropertyChangedEventArgs.new(name) if ev.respond_to? :call
      end
    end
